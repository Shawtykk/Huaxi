...
parser.add_argument('--model_path', default = 'Swin-Unet/output_BVR/epoch_299.pth',type=str)  
parser.add_argument('--output_dir', default = 'Swin-Unet/outPut_task3',type=str, help='output dir')                   
parser.add_argument('--data_path', default = 'Swin-Unet/data_BVR/images',type=str, help='data_path image dir or an single image')
...
def visualization(output,img_name):
    ...
    path = osp.join('result_bvr','v_' + img_name)
    outputImg.save(path)  
    return path
...
def Rule_based(output, img_name, angle = 0):
    ...
    for an in range(-angle, angle + 1):
        ...
        w1_index = []
        for i in w1_axis:
            w1_index.append(w1_axis.index(i))
        w1_index = list(set(w1_index))
        w1_index = np.array(w1_index)
        w1_index = sorted(w1_index,reverse=False)
       
        w1_new = lugu[:,0][w1_index]
        h1_new = h1_axis[w1_index]
        if(flag == 1):
            w1_index = list(w1_new).index(w_new[index_min[-1]])
            h1 = h_new[index_min[-1]] - h1_new[w1_index]
            arr1 = np.full(h1,w_new[index_min[-1]])
            arr1_y = array(range(h1_new[w1_index], h_new[index_min[-1]]))
        else:
            w1_index = list(w1_new).index(w_new[index_min[0]])
            h1 = h_new[index_min[0]] - h1_new[w1_index] 
            arr1 = np.full(h1,w_new[index_min[0]])
            arr1_y = array(range(h1_new[w1_index], h_new[index_min[0]]))
        ratio = h1/h2
        ...
        zoom_path=osp.join('result_bvr_zoom','v_' + img_name)       
        cv2.imwrite(zoom_path, zoom_im)
        zoomori_path=osp.join('result_bvr_zoomori','v_' + img_name)
...
def main():
    ...
    model.load_state_dict(torch.load('Swin-Unet/output_BVR/epoch_299.pth'))
    model.eval()
    gt_scores = json.load(open('Swin-Unet/data_BVR/task34_gt.json'))
    error = 0
    true_positive = 0
    true_negative = 0
    false_positive = 0
    ...
    for path in tqdm(img_paths):
        ...
        gt_score = float(gt_scores[img_name]['res1'] / gt_scores[img_name]['res2'])
        ...
        # 写json文件
        new_data = {"gt_BVR":gt_score,"result_BVR":score }
        with open('Swin-Unet/data_BVR/task34_gt.json','r', encoding="utf-8") as f:
            old_data = json.load(f)           
            old_data[img_name].update(new_data)
        with open('Swin-Unet/data_BVR/task34_gt.json','w', encoding="utf-8") as f:
            json.dump(old_data, f,indent=1)
    
        if score < threshold and gt_score < 1.0:
            true_positive += 1
            print("11: " + img_name)
        elif score >= threshold and gt_score < 1.0:
            false_positive += 1
            print("12: " + img_name)
        elif score < threshold and gt_score >= 1.0:
            false_negative += 1
